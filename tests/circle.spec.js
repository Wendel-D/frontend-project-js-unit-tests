...
describe('4 - Implemente os casos de teste para a função `circle`', () => {
  it('Verifica se ao receber um raio, a função `circle` retorna um objeto contendo os valores esperados', () => {
    const expectedOutput = {radius: 1, area: 3.14, circumference: 6.28};
    expect(circle(1)).toEqual(expectedOutput);
  });
  ...
  it('Teste se dentro do objeto retornado, a função retorna uma `key` com `value` igual à circunferência correta para um círculo de raio 2.', () => {
    const expectedOutput = {area: 12.56};
    expect(circle(2)).toMatchObject(expectedOutput);
  })
  it('Teste se dentro do objeto retornado, a função retorna uma `key` com `value` igual à área correta para um círculo de raio 3.', () => {
    const expectedOutput = {area: 28.259999999999998};
    expect(circle(3)).toMatchObject(expectedOutput);
  })
